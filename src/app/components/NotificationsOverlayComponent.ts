...
@Component({
    ...
    template: `
        <div class="notification-holder">
            <div *ngIf="notificationService.highImportanceOverlay"
                 [class.open]="notificationService.highImportanceOverlayOpen" class="notification-overlay"></div>
            <div class="notification {{notification.visit()}}"
                 *ngFor="let notification of notificationService.overlayNotifications"
                 [class.n-high-importance]="notification.highImportance" [class.n-state-open]="notification.opened"
                 [class.n-state-close]="notification.closed">
                <div class="notification-inner n-{{notification.type}}" [class.n-state-over]="notification.overed"
                     (mouseover)="onOver(notification)" (mouseout)="onOut(notification)">
                    <i class="n-icon fa fa-{{notification.icon}}"></i>
                    <button
                        *ngIf="!notification.highImportance || !(notification.buttons && notification.buttons.length)"
                        type="button" class="n-close" (click)="onCloseClick(notification, $event)"><i
