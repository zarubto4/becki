...
export class Component implements ng.OnInit {
  ...
  onSubmit():void {
    ...
    this.backEnd.createProcessor(this.nameField, this.codeField, this.descriptionField, this.speedField)
        .then(() => {
          this.notifications.next.push(new notifications.Success("The processor has been created."));
          this.router.navigate(["Devices"]);
        })
        .catch(reason => {
          this.notifications.current.push(new notifications.Danger("The processor cannot be created.", reason));
        });
  }
