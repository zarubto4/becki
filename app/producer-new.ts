...
export class Component implements ng.OnInit {
  ...
  onSubmit():void {
    ...
    this.backEnd.createProducer(this.nameField, this.descriptionField)
        .then(() => {
          this.notifications.next.push(new notifications.Success("The producer has been created."));
          this.router.navigate(["Devices"]);
        })
        .catch(reason => {
          this.notifications.current.push(new notifications.Danger("The producer cannot be created.", reason));
        });
  }
