...
export class Component implements ng.OnInit {
  ...
  onSubmit():void {
    ...
    this.backEnd.createLibrary(this.nameField, this.descriptionField)
        .then(() => {
          this.notifications.next.push(new notifications.Success("The library has been created."));
          this.router.navigate(["Devices"]);
        })
        .catch(reason => {
          this.notifications.current.push(new notifications.Danger("The library cannot be created.", reason));
        });
  }
